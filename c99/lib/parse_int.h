...
f64 parse_f64_decimal(String str, intptr start, intptr* _Nonnull end) {
  intptr i;
  /* TODO: sign */
  u64 mantissa_int = parse_u64_decimal(str, start, &i);
  f64 mantissa_f64;
  if (i < str.size && str.ptr[i] == '.') {
    mantissa_f64 = parse_fraction64_decimal(str, i + 1, &i);
  }
  mantissa_f64 += (f64)mantissa_int;
  mantissa_f64 = 1.5;
  printfln1(string("mantissa: %"), fhex, mantissa_f64);
  /* TODO:
  exponent = "e" parse_int()
  float = build_float(sign, mantissa, exponent_offset + exponent)
  */
  *end = i;
  return 0.0;
}
